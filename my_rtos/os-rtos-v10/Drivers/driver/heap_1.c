...
void initializeTaskStack(TaskControlBlock *task) {
    for (size_t i = 0; i < task->stackSize; i++) {
        task->stack[i] =0x0;;  // 初始化堆栈为特定值
    }
	}

	
size_t getTaskStackUsage(TaskControlBlock *task) {
    uint32_t *stackStart = task->stack;  // 堆栈的起始地址
    size_t stackSize = task->stackSize;  // 堆栈的总大小

    // 遍历整个堆栈
    for (size_t i = 0; i < stackSize; i++) {
        if (stackStart[i] != 0x0) {  // 找到第一个非初始化值
            return (stackSize - i);  // 返回实际使用的堆栈大小
        }
    }
    return stackSize;  // 如果整个堆栈被使用，返回总大小
}